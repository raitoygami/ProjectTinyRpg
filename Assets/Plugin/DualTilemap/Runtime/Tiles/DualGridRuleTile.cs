...
namespace skner.DualGrid
{
    ...
    public class DualGridRuleTile : RuleTile<DualGridNeighbor>
    {
        ...
        public override void GetTileData(Vector3Int position, ITilemap tilemap, ref TileData tileData)
        {
            ...
            // 只在编辑器里、且当前是 Palette 时强制用预览图
#if UNITY_EDITOR
            if (IsTilemapFromPalette(tilemap) && m_DefaultSprite != null)
            {
                tileData.sprite = m_DefaultSprite;
                // 如果只想显示预览、不实例化 GameObject，可再清掉：
...
#if UNITY_EDITOR
        // 判断是否是 Tile Palette 的 Tilemap（常用写法）
        private static bool IsTilemapFromPalette(ITilemap itilemap)
        {
            var tm = itilemap.GetComponent<Tilemap>();
            if (tm == null) return false;

            // 方法一：名字里通常带 "Palette"
            if (tm.name.Contains("Palette")) return true;

            // 方法二：层级（很多项目把 Palette 放在 Layer 31）
            // if (tm.gameObject.layer == 31) return true;

            // 方法三：父物体是 Grid 且 hideFlags 特殊
            var grid = tm.GetComponentInParent<Grid>();
            return grid != null && (grid.hideFlags & HideFlags.HideAndDontSave) != 0;
        }
#endif
